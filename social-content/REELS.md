...
El original va a `Materiales/reels/<reel>/video/`; el parcheado a
`social-content/public/videos/`. **Se les pone el mismo nombre a los dos** para que la
...
### Carrusel (posts de una subasta puntual) — efímero

| Carpeta | Qué es | ¿Al repo? |
|---|---|---|
| `Materiales/<id>/` | fotos que bajó el scraper | **no** (`.gitignore`) |
| `Materiales/cierre.png`, `logo.svg`, flechas | placas y flechas del carrusel | sí |
| `social-content/public/autos/` | las mismas fotos, servidas a Remotion | **no** |
| `Posts/<slug>/` | los PNG finales | **no** |
| `src/posts/`, `src/subasta.ts` | la composición `Auto` | sí |
| `nueva-subasta.sh`, `scraper.py`, `ajustar.sh` | el pipeline | sí |
...
| Carpeta | Qué es | ¿Al repo? |
|---|---|---|
| `Materiales/reels/<reel>/video/` | las tomas **originales** del generador | **sí** |
| `Materiales/reels/<reel>/voz/` | la locución original de ElevenLabs | **sí** |
| `Materiales/marca/` | iconos de marca en su tamaño original | **sí** |
| `social-content/public/videos/` | las tomas ya parcheadas que usa el reel | **sí** |
| `social-content/public/voz/` | las locuciones que usa el reel | **sí** |
| `social-content/public/reel/` | fotos fijas y fondos que usan los reels | **sí** |
| `social-content/public/personas/`, `frames/` | recortes y SVG de los reels | **sí** |
| `social-content/public/brand/` | logo, tipografías, iconos | **sí** |
| `src/reels/`, `*-VENDESOLO.md` | el código y los documentos | **sí** |
| `social-content/out/` | los mp4 renderizados | **no**, se regeneran |
...
**La trampa que ya nos mordió:** `Materiales/` en su raíz es de donde `nueva-subasta.sh` toma las
fotos sueltas de un carrusel. Si dejas un PNG de un reel ahí tirado, el script se lo lleva al
carrusel. Por eso todo lo de reel vive en `Materiales/reels/` y la raíz sólo tiene las cuatro cosas
del carrusel.
